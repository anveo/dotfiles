...
**"In the chosen worktree" means a session whose working directory is that worktree — not this one reaching into it.** `wta -w` starts a Claude session in each ticket's tmux window for exactly this reason. A dispatching session that builds instead resolves the main checkout's `.envrc` (wrong ports, wrong hostname, wrong database), runs against whatever branch the main checkout has out, and has to re-specify the worktree path on every call — where a single omission puts the edit or the commit in the main checkout. It also cannot be in more than one of these at once, which is the whole point of dispatching several.

**It is implementation that moves, not the conversation.** Answer the questions right here if that is where you are — this session is holding the brief that raised them, and asking someone to carry their own answers to another window is friction with nothing on the other end of it. Write the answers into the issue as a comment, the way `preflight` writes back everything else, and they reach the worktree's session without passing through anyone's memory. The exception is a multi-issue dispatch, where answering in each ticket's own window beats making this one a relay for four sets of answers at once.

What does not happen here is the building. Do not run `takeoff` here and do not start implementing here, however complete the brief in front of you feels and however fully the questions have just been settled — a session that keeps taking input tends to keep going, and the answers arriving is exactly the moment that temptation peaks. The `worktree-guard` hook denies writes and test runs against a worktree from outside it, so this is enforced rather than merely asked for — but arriving at that denial means the handoff was already missed. If a single session genuinely should own one worktree, `EnterWorktree({path: …})` moves it there properly instead of prefixing every command.
