...
class AlignSelectionToGPencil(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        source = bpy.context.scene.tool_settings.grease_pencil_source
        if(source == 'SCENE'):
            if bpy.context.scene.grease_pencil.layers[-1].active_frame is None:
                return False
        elif (source == 'OBJECT'):
            if bpy.context.active_object.grease_pencil.layers[-1].active_frame is None:
                return False
        else:
            return True
...
def clear_gpencil_strokes(context):
    if context.user_preferences.addons[__name__].preferences.clear_strokes:
        source = bpy.context.scene.tool_settings.grease_pencil_source
        if(source == 'SCENE'):
            bpy.context.scene.grease_pencil.layers[-1].active_frame.clear()
        elif (source == 'OBJECT'):
...
def gpencil_to_screenpos(context):
    ...
    if(source == 'SCENE'):
        gps = bpy.context.scene.grease_pencil.layers[-1].active_frame
    elif (source == 'OBJECT'):
